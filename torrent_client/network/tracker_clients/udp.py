...
class UDPTrackerClient(BaseTrackerClient):
    ...
    async def announce(self, server_port: int, event: EventType):
        transport, protocol = await self._loop.create_datagram_endpoint(
            # DatagramReaderProtocol, remote_addr=("localhost", 6969))
            DatagramReaderProtocol, remote_addr=(self._host,self._port))
        try:
            local_ip = transport.get_extra_info('sockname')
            logger.debug('announce via udp://%s:%s from %s (event=%s)',
                         self._host, self._port, local_ip, event.name)
            ...
            logger.debug('announce response: interval=%s leechers=%s seeders=%s peers=%s',
                         self.interval, self.leech_count, self.seed_count, len(self._peers or []))
        finally:
            transport.close()
